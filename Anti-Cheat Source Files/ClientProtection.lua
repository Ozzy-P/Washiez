...
l__Parent__2:GetPropertyChangedSignal("Disabled"):Connect(function()
	...
	u3("AE Kicking Player (Client Tampering)");
end);
l__Parent__2:GetPropertyChangedSignal("Parent"):Connect(function()
	...
	u3("AE Kicking Player (Client Tampering 2)");
end);
script.Parent.DescendantRemoving:Connect(function(p1)
	...
	u3("AE Kicking Player (Client Tampering 3)");
end);
local u4 = false;
spawn(function()
	while true do
		wait(0.3);
		if (not v5 or not v6) and not u4 then
			u4 = true;
			if v1 then
				u3("AE Kicking Player (Event Tampering)");
			else
				v5:FireServer("BanMe", "Tempering with the Anti-Exploit");
				l__LocalPlayer__3:Kick("Do not tamper with the Anti-Exploit");
			end;
		end;
		if (not v5:IsDescendantOf(game:GetService("ReplicatedStorage")) or not v6:IsDescendantOf(game:GetService("ReplicatedStorage"))) and not u4 then
			u4 = true;
			if v1 then
				u3("AE Kicking Player (Event Tampering 2)");
			else
				v5:FireServer("BanMe", "Tempering with the Anti-Exploit");
				l__LocalPlayer__3:Kick("Do not tamper with the Anti-Exploit");
			end;
